...
const HomeTestomonialsUp = () => {
    const { activeIndex, goTo, next, prev, getOffset, pause, resume } =
        useTestimonialCarousel({ length: homeTestimonialsData.length, autoPlayMs: 3500 })
    ...
    useEffect(() => {
        homeTestimonialsData.forEach((t, i) => {
            prevOffsets.current.set(t.id, getOffset(i))
        })
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [activeIndex])

    // keep the active dot in view as the strip scrolls horizontally
    // useEffect(() => {
    //     dotRefs.current.get(activeIndex)?.scrollIntoView({
    //         behavior: "smooth",
    //         block: "nearest",
    //         inline: "center",
    //     })
    ...
    useEffect(() => {
        if (isFirstRender.current) {
            ...
            return // don't scroll anything on mount — only respond to real index changes
        }
        ...
        const target =
            dot.offsetLeft - strip.clientWidth / 2 + dot.clientWidth / 2
            ...
            <RevealOnScroll delay={150}>
                ...
                >
                    <span className="sr-only" aria-live="polite">
                        Showing testimonial {activeIndex + 1} of {homeTestimonialsData.length}
                    </span>

                    {homeTestimonialsData.map((testimonial, index) => {
                        ...
                        const skipTransition = wrapped || prefersReducedMotion
                        ...
                        return (
                            ...
                            >
                                <HomeTestimonialsCard testimonial={testimonial} isActive={isActive} />
                            </div>
                        )
                    })}
                </div>
                ...
                <div className="mt-8 flex items-center justify-center gap-6">
                    ...
                    <div
                        ref={stripRef}
                        className="no-scrollbar flex max-w-[220px] items-center gap-2 overflow-x-auto scroll-smooth px-1 sm:max-w-none"
                    >
                        {homeTestimonialsData.map((testimonial, index) => (
                            <button
                                key={testimonial.id}
                                ...
                                className={cn(
                                    "h-2 shrink-0 rounded-full cursor-pointer default-transition",
                                    index === activeIndex
                                        ? "w-6 bg-primary"
                                        : "w-2 bg-border-clr hover:bg-text-secondary-muter"
                                )}
                            />
                        ))}
                    </div>
...
export default HomeTestomonialsUp