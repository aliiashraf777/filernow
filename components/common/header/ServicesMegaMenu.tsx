...
const categoryMeta: Record<IServiceItem["category"], string> = {
    registration: "Registration Services",
    compliance: "Compliance & Filing",
    licenses: "Licenses & Certifications",
}
const categoryOrder: IServiceItem["category"][] = ["registration", "compliance", "licenses"]
                ...
                {grouped.map(({ category, items }) => (
                    <div key={category}>
                        ...
                        <ul className="flex flex-col gap-3.5">
                            {items.map((item) => (
                                <li key={item.id}>
                                    <Link href={item.href} onClick={onNavigate} className="flex items-center gap-2 para-base text-text-secondary hover:text-primary default-transition">
                                        <span
